...
def currentVersion() {
    cur_ver = sh(
        returnStdout: true,
        script: "cat mist/__version__.py | grep __version__ | cut -d \"'\" -f 2"
    ).trim()
    return cur_ver
}
...
node("JenkinsOnDemand") {
    ...
    stage('Build') {
        ...
        sh "virtualenv mist-cli-p3 -p python3"
        sh ". ./mist-cli-p3/bin/activate && pip install --upgrade pip && pip install coverage codecov && pip install ."
    }

    stage('Test') {
        sh ". ./mist-cli-p3/bin/activate && coverage run --source mist setup.py test"

        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'CODECOV_TOKEN', usernameVariable: 'CODECOV_TOKEN_NAME', passwordVariable: 'CODECOV_TOKEN']]) {
            sh ". ./mist-cli-p3/bin/activate && codecov -t ${CODECOV_TOKEN}"
        }
    }
    ...
    if (isReleaseJob()) {
        ...
        stage('Push to PYPI') {
            sh ". ./mist-cli-p3/bin/activate && python setup.py sdist"
            sh ". ./mist-cli-p3/bin/activate && pip install twine"
            configFileProvider([configFile(fileId: 'PYPIDeployConfiguration', targetLocation: '.pypirc', variable: 'PYPI_SETTINGS')]) {
                sh ". ./mist-cli-p3/bin/activate && twine upload --config-file ${env.WORKSPACE}/.pypirc -r pypi ${env.WORKSPACE}/dist/*"
            }
        }
